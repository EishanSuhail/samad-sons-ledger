...
const BT = String.fromCharCode(96); // Backtick character to avoid escape/syntax issues
...
if (!html.includes(targetHtml)) {
  console.log("Warning: Target API key container not found in index.html, checking if already patched...");
  if (html.includes('gemini-api-key')) {
    console.log("index.html is already patched for Gemini!");
  } else {
    console.error("Could not find the target API Key HTML section in index.html.");
    process.exit(1);
  }
} else {
...
if (app.includes(targetInitVoice)) {
  ...
  console.log("initVoiceStudio patched!");
} else if (app.includes('gemini-api-key')) {
  console.log("initVoiceStudio already patched.");
} else {
  console.error("Could not find initVoiceStudio target in app.js");
  process.exit(1);
}

// 2b. Re-write handleOcrImageCapture
...
// We build newOcrCapture string dynamically using String.fromCharCode(96) to completely avoid escape issues.
const newOcrCapture = `async function handleOcrImageCapture(event) {
  ...
  try {
    ...
    if (geminiKey) {
      ...
      const response = await fetch(` + BT + `https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=\${geminiKey}` + BT + `, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
      ...
      const response = await fetch('https://api.openai.com/v1/chat/completions', {
        ...
        headers: {
          'Content-Type': 'application/json',
          'Authorization': ` + BT + `Bearer \${openaiKey}` + BT + `
        },
        body: JSON.stringify({
          model: "gpt-4o-mini",
          messages: [
...
if (app.includes(targetOcrCapture)) {
  ...
  console.log("handleOcrImageCapture already contains Gemini code.");
} else {
  console.error("Could not find targetOcrCapture target in app.js");
  process.exit(1);
}

// 2c. Append saveGeminiKey function to end
...
if (!app.includes('function saveGeminiKey(')) {
  app = app + "\n" + saveGeminiKeyFunc;
  console.log("saveGeminiKey function appended!");
} else {
  console.log("saveGeminiKey already present.");
}

app = app.replace(/\n/g, '\r\n'); // Convert back to CRLF
fs.writeFileSync(appPath, app, 'utf8');
console.log("app.js completely patched successfully!");
