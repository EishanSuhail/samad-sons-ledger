...
// Target the exact Gemini API fetch block inside handleOcrImageCapture
const targetGeminiBlock = `    if (geminiKey) {
      ...
      if (cleanJson.startsWith('\`\`\`')) {
        cleanJson = cleanJson.replace(/^\\s*\\\`\\\`\\\`json\\s*/i, '').replace(/\\\`\\\`\\\`$/, '').trim();
      }
...
const newGeminiBlock = `    if (geminiKey) {
      // Clean base64 prefix for Gemini API
      const base64Clean = base64Img.split(',')[1];
      const mimeType = base64Img.split(';')[0].split(':')[1];
      ...
      // Dynamic fallback list to support all API keys (old and new)
...
if (!app.includes(targetGeminiBlock)) {
  console.error("targetGeminiBlock in app.js not found!");
  process.exit(1);
}

app = app.replace(targetGeminiBlock, newGeminiBlock);

app = app.replace(/\n/g, '\r\n'); // Convert back to CRLF
fs.writeFileSync(appPath, app, 'utf8');
console.log("app.js successfully patched for dynamic Gemini resilience!");
